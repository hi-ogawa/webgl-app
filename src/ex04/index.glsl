...
float sdJulia(vec2 z, vec2 c, mat2 dpdw) {
  vec2 dz = C_1;
  ...
  float sd = g / length(dg * dpdw);  // factor 2^k cancels here
  return sd;
}
...
#define MIN_ASSIGN(X_OUT, D_OUT, X, D) \
  ...
  vec4 renderPixel(vec2 frag_coord) {
    float height = 2.5;
    mat3 inv_view_xform =
        T_invViewXform(height, U_resolution)
        * T_scale(vec2(pow(2.0, U_view_zoom)))
        * T_translate(U_view_xy);
    ...
    vec3 color1;
    {
      ...
      color1 = mixGamma(OZN.yyy, result * 0.8, fac, 2.2);
    }
    ...
    vec3 color2;
    {
      ...
      float fac = sdToFactor(sd, 2.0);
      color2 = mixGamma(OZN.yyy, vec3(0.0, 1.0, 1.0), fac, 2.2);
    }
